...
type authPolicyView struct {
	...
	HeaderClaim string   `json:"header_claim"`
...
func (s *Server) handleListAuthPolicies(w http.ResponseWriter, r *http.Request) {
	...
	for _, p := range s.cfg.AuthPolicy {
		out = append(out, authPolicyView{
			ID: p.ID, Methods: p.Methods, ClaimName: p.ClaimName, ClaimValue: p.ClaimValue,
			Header: p.Header, HeaderClaim: p.HeaderClaim, Username: p.Username,
...
func (s *Server) handleSaveAuthPolicy(w http.ResponseWriter, r *http.Request) {
	...
	for _, m := range req.Methods {
		if m != config.AuthMethodSystem && m != config.AuthMethodPassword {
			http.Error(w, fmt.Sprintf("unknown method %q", m), http.StatusBadRequest)
			return
		}
	}
	...
	pol := config.AuthPolicy{
		ID: req.ID, Methods: req.Methods, ClaimName: req.ClaimName, ClaimValue: req.ClaimValue,
		Header: req.Header, HeaderClaim: req.HeaderClaim, Username: req.Username,
	}
	updated := false
