...
The installer downloads the binary, generates a config, creates a systemd service, and starts Gatecrash.
...
HTTP tunnels support additional configuration options:

| Option | Default | Description |
|--------|---------|-------------|
| `preserve_host` | `false` | Pass the original `Host` header to the backend instead of rewriting it to the target address. Useful when the backend application needs to see the original hostname. |
| `tls_passthrough` | `false` | Forward raw TLS connections without terminating them. The backend handles TLS instead of Gatecrash. When enabled, Gatecrash will not provision Let's Encrypt certificates for these hostnames. |
